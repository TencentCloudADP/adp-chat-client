...
def to_message(
    ...
    Args:
        type: 消息类型 / Message type:
            - REPLY, THOUGHT, REFERENCE, TOKEN_STAT: 需要`record`参数，可选`incremental`参数 / require `record`, optional `incremental`
            - CONVERSATION: 需要`conversation`参数，可选`is_new_conversation`参数 / requires `conversation`, optional `is_new_conversation`
            - ERROR: 需要`error_msg`参数 / requires `error_msg`
