...
<PropsTable data={[
  ...
  { name: 'widgetBasePath', type: 'string', default: "'/static/adp-chat-component/umd/widget'", description: 'Widget SDK base path, supports absolute path or CDN URL' },
...
The chat component supports rendering custom Widgets (such as charts, forms, and other interactive components). Widget SDK resources are loaded from `/static/adp-chat-component/umd/widget` path by default.

### Loading Widget from CDN

If you want to load Widget SDK resources from a CDN for better performance, you can configure `widgetBasePath`:

```javascript
ADPChatComponent.init('#chat-container', {
  widgetBasePath: 'https://cdn.example.com/widget',
  apiConfig: {
    baseURL: 'https://api.example.com'
  }
})
...
If you deploy Widget resources to a custom path, you can also specify it via `widgetBasePath`:

```javascript
ADPChatComponent.init('#chat-container', {
  widgetBasePath: '/assets/widget',
  apiConfig: {
    baseURL: 'https://api.example.com'
  }
})
...
The Widget SDK contains the following files that need to be deployed to the directory specified by `widgetBasePath`:

| Filename | Description |
| --- | --- |
| `adp-widget.js` | Widget core SDK |
| `adp-widget-vendor.js` | Third-party dependencies (Vue, etc.) |
| `adp-widget-chart.js` | Chart component (ECharts) |
| `adp-widget-markdown.js` | Markdown rendering component |
