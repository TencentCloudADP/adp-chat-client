...
4. File Preview Service: To preview Word, Excel, PPT and other office documents, you need to enable the document processing service.
   1. Log in to the [Tencent Cloud COS Console](https://console.cloud.tencent.com/cos/bucket) with the root account;
   2. Search for the COS bucket name configured in `.env` as `COS_BUCKET` (default: `chat-client-bucket-${TC_SECRET_APPID}`, note: `${TC_SECRET_APPID}` refers to the `TC_SECRET_APPID` value in your config, e.g. the actual bucket name would be `chat-client-bucket-1322044278`), and click to open it;
   3. In the left menu, navigate to **Data Processing** → **Document Processing** → **Enable**;
   4. **Data Processing** → **File Processing** → **Enable**.
...
* **Q: The page shows "Unknown Application."**
