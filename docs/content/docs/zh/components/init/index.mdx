...
<PropsTable data={[
  ...
  { name: 'widgetBasePath', type: 'string', default: "'/static/adp-chat-component/umd/widget'", description: 'Widget SDK 的基础路径，支持绝对路径或 CDN 地址' },
...
聊天组件支持渲染自定义 Widget（如图表、表单等交互式组件）。Widget SDK 资源默认从 `/static/adp-chat-component/umd/widget` 路径加载。

### 使用 CDN 加载 Widget

如果您希望从 CDN 加载 Widget SDK 资源以提升加载速度，可以配置 `widgetBasePath`：
...
如果您将 Widget 资源部署到自定义路径，也可以通过 `widgetBasePath` 指定：
...
Widget SDK 包含以下文件，需要部署到 `widgetBasePath` 指定的目录下：

| 文件名 | 说明 |
| --- | --- |
| `adp-widget.js` | Widget 核心 SDK |
| `adp-widget-vendor.js` | 第三方依赖（Vue 等） |
| `adp-widget-chart.js` | 图表组件（ECharts） |
| `adp-widget-markdown.js` | Markdown 渲染组件 |
